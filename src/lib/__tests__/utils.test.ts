...
import { parseStrikeFromSymbol, isExcludedSector, getTickerList } from "../utils";
...
describe("isExcludedSector", () => {
  ...
  it("excludes by industry (case-insensitive partial match)", () => {
    ...
    expect(isExcludedSector("Mortgage Real Estate Investment Trusts (REITs)", null)).toBe(true);
  });
