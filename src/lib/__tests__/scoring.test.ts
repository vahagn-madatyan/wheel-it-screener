...
describe("computeWheelMetrics", () => {
  ...
  it("computes buyingPower as price × 100", () => {
    const result = computeWheelMetrics(makeStock({ price: 42.5 }), defaultFilters);
  ...
  it("clamps premiumYield to 0-100", () => {
    ...
    const result = computeWheelMetrics(
      makeStock({ price: 37, beta: 5.0 }),
      { targetDelta: 0.5, targetDTE: 7 },
    );
...
describe("computeWheelScore", () => {
  ...
  it("computes fundamentalsScore PE tiers correctly", () => {
    ...
    const r1 = computeWheelScore(
      makeStock({ pe: 8, dividendYield: 0, roe: null, netMargin: null, premiumYield: 10 }),
      defaultWeights,
    );
    ...
    const r2 = computeWheelScore(
      makeStock({ pe: 25, dividendYield: 0, roe: null, netMargin: null, premiumYield: 10 }),
      defaultWeights,
    );
    ...
    const r3 = computeWheelScore(
      makeStock({ pe: 50, dividendYield: 0, roe: null, netMargin: null, premiumYield: 10 }),
      defaultWeights,
    );
