...
describe('filterStocks', () => {
  ...
  it('filters by net margin when minNetMargin is defined', () => {
    ...
    const result = filterStocks([stock], makeFilters({ minNetMargin: 0 }));
    expect(result.length).toBe(0);
  });
  ...
  it('filters by sales growth', () => {
    ...
    const result = filterStocks([stock], makeFilters({ minSalesGrowth: 5 }));
    expect(result.length).toBe(0);
  });
  ...
  it('filters by SMA200', () => {
    ...
    const result = filterStocks([stock], makeFilters({ aboveSMA200: true }));
    expect(result.length).toBe(0);
  });
  ...
  it('handles all candidates filtered out', () => {
    ...
    const stocks = [makeStock({ pe: 70 }), makeStock({ pe: 80 })];
